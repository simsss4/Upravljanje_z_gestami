...
# Truncate sequences
max_frames = 80
X_radio = X_radio[:, :max_frames, :]  # Shape: (61, 80, 63)
...
X_radio = compute_finger_distances(X_radio)  # Shape: (61, 80, 72)
...
X_radio_aug1 = augment_landmarks(X_radio)
X_radio_aug2 = augment_landmarks(X_radio)
X_radio_aug3 = augment_landmarks(X_radio)
X_radio = np.concatenate([X_radio, X_radio_aug1, X_radio_aug2, X_radio_aug3], axis=0)
y_radio = np.concatenate([y_radio, y_radio, y_radio, y_radio], axis=0)
print("Augmented X_radio shape:", X_radio.shape)  # Expected: (244, 80, 72)
...
def create_radio_model():
    model = models.Sequential([
        layers.InputLayer(shape=(max_frames, 72)),  # Updated for new features
...
for train_idx, val_idx in kf.split(X_radio):
    print(f"\nFold {fold}")
