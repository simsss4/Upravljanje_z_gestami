...
# Load only radio data, labels from 0
data_radio, labels_radio, mapping_radio = load_data_from_directory('data_radio', start_label=0)

# Procesiraj
max_length_radio = max(len(d) for d in data_radio)
X_radio = process_data(data_radio, max_length=max_length_radio)
y_radio = labels_radio

# Shrani
np.save('X_radio.npy', X_radio)
np.save('y_radio.npy', y_radio)

print("RADIO label mapping:", mapping_radio)
print(f"X_radio shape: {X_radio.shape}, y_radio shape: {y_radio.shape}")
