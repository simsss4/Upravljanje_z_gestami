...
# Load data
x_windows = np.load('x_windows.npy')  # Shape: (40, 698, 63)
y_windows = np.load('y_windows.npy')  # Shape: (40,)

# Analyze sequence length
non_zero_frames = np.sum(np.any(x_windows != 0, axis=2), axis=1)
...
x_windows_aug1 = augment_landmarks(x_windows)
x_windows_aug2 = augment_landmarks(x_windows)
x_windows = np.concatenate([x_windows, x_windows_aug1, x_windows_aug2], axis=0)
y_windows = np.concatenate([y_windows, y_windows, y_windows], axis=0)
print("Augmented x_windows shape:", x_windows.shape)  # Expected: (120, 80, 63)
...
def plot_gestures(X, y):
    ...
    for i in range(8):
        ...
        ax1.plot(X[idx, :, 0], label=f'Class {i}: {["close_back_left_window", "close_back_right_window", "close_front_left_window", "close_front_right_window", "open_back_left_window", "open_back_right_window", "open_front_left_window", "open_front_right_window"][i]}')
...
plot_gestures(x_windows, y_windows)

# Model architecture (simplified LSTM)
def create_windows_model():
...
for train_idx, val_idx in kf.split(x_windows):
    ...
    print(classification_report(y_val, y_pred, labels=[0, 1, 2, 3, 4, 5, 6, 7],
                               target_names=['close_back_left_window', 'close_back_right_window', 'close_front_left_window', 'close_front_right_window', 'open_back_left_window', 'open_back_right_window', 'open_front_left_window', 'open_front_right_window'],
                               zero_division=0))
    fold += 1
...
print(confusion_matrix(y_val_final, y_pred_final, labels=[0, 1, 2, 3, 4, 5, 6, 7]))
print("Final Classification Report:")
print(classification_report(y_val_final, y_pred_final, labels=[0, 1, 2, 3, 4, 5, 6, 7],
                           target_names=['close_back_left_window', 'close_back_right_window', 'close_front_left_window', 'close_front_right_window', 'open_back_left_window', 'open_back_right_window', 'open_front_left_window', 'open_front_right_window'],
                           zero_division=0))

# Save final model
model.save('windows_model.keras')
